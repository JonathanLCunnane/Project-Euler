...
# endregion
def is_consecutive(values) -> bool:
    return (values[0] == values[1] + 1 and values[1] == values[2] + 1 and values[2] == values[3] + 1 and values[3] == values[4] + 1)
def hand_values(hand) -> list:
    # royal flush is 9, straight flush is 8, four of a kind is 7, etc.. high card is 0
    # the list returned will have the hand value, then the card individual values after 
    # e.g. for 2H 2D 4C 4D 4S -> [6, 4, 4, 4, 2, 2] (full house and then the values of the cards)
    
    # translate T -> 10, J -> 11, Q -> 12, K -> 13, A -> 14
    for index, card in enumerate(hand):
        if card[0] == "T":
            hand[index] = "10" + hand[index][1]
            continue
        elif card[0] == "J":
            hand[index] = "11" + hand[index][1]
            continue
        elif card[0] == "Q":
            hand[index] = "12" + hand[index][1]
            continue
        elif card[0] == "K":
            hand[index] = "13" + hand[index][1]
            continue
        elif card[0] == "A":
            hand[index] = "14" + hand[index][1]
            continue
    values = [int(card[:-1]) for card in hand]
    values.sort(reverse=True)
    suit_list = [card[-1] for card in hand]
    suits = dict() # contains the suit then the number, e.g. hand of 5D 8C 9S JS AC would have suits of {"D": 1, "C": 2, "S": 2}
    for card in hand:
        if card[-1] not in suits:
            suits[card[-1]] = suit_list.count(card[-1])
        else:
            continue
    # find the hand value
    # firstly royal flush
    number_of_suits = len(suits)
    num_of_first = values.count(values[0])
    num_of_second = values.count(values[1])
    num_of_third = values.count(values[2])
    num_of_fourth = values.count(values[3])
    if number_of_suits == 1 and values == [14, 13, 12, 11, 10]:
        values.insert(0, 9)
    # straight flush
    elif number_of_suits == 1 and is_consecutive(values):
        values.insert(0, 8)
    # four of a kind
    elif num_of_first == 4 or num_of_second == 4:
        values.insert(0, 7)
    # full house
    elif (num_of_first == 3 and num_of_fourth == 2) or (num_of_first == 2 and num_of_third == 3):
        values.insert(0, 6)
    # flush
    elif number_of_suits == 1:
        values.insert(0, 5)
    # straight
    elif is_consecutive(values):
        values.insert(0, 4)
    # three of a kind
    elif num_of_first == 3 or num_of_second == 3 or num_of_third == 3:
        values.insert(0, 3)
    # two pairs
    elif (num_of_first == 2 and num_of_third == 2) or (num_of_first == 2 and num_of_fourth == 2) or (num_of_second == 2 and num_of_fourth == 2):
        values.insert(0, 2)
    # pair
    elif num_of_first == 2 or num_of_second == 2 or num_of_third == 2 or num_of_fourth == 2:
        values.insert(0, 1)
    # high card
    else:
        values.insert(0, 0)
    return values
def h1_winning(hand_1, hand_2) -> bool:
    hand_1 = hand_values(hand_1)
    hand_2 = hand_values(hand_2)
    h1_type = hand_1[0]
    h2_type = hand_2[0]
    ...
    # if one pair
        ...
        if h2_type != 1:
            return False
        h1_pair = [val for val in hand_1[1:] if hand_1[1:].count(val) == 2][0]
        h2_pair = [val for val in hand_2[1:] if hand_2[1:].count(val) == 2][0]
        # oops got bored
hands = hands.split("\n")
games = [[game[:14].split(" "), game[15:].split(" ")] for game in hands]
wins_p1 = 0
#region
wins_p1 = 376
#endregion
...
for game in games[0]:
    ...
    hand_p2 = game[1]
    break
    if h1_winning(hand_p1, hand_p2):
        wins_p1 += 1
end = (((perf_counter() - start)*100000)//1)/100
print(f"Player 1 Won {wins_p1} times, this took {end}ms.")