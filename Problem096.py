...
def soln_available(grid: list[list[int]]) -> bool or int:
    for row in grid:
        if row.count(0) != 0:
            return False
    return int("".join([str(n) for n in grid[0][:3]]))
...
grids = grids.splitlines()
grids = [[[int(val) for val in line] for line in grids[x:x+9]] for x in range(1, len(grids), 10)]
grid_sum = 0
start = perf_counter()
...
for grid in grids:
    ...
    soln_found = False
    while not (deduction_complete or soln_found):
    ...
    while solved_cells < 81:
        ...
        try:
            ...
            print(cell_pointer_idxs, pointeridx)
            print(all_cell_values, pointeridx)

    soln = soln_available(grid)
    grid_sum += soln
    print(f"Solution forced for: {grids.index(grid)+1}, {soln}")

end = perf_counter()
elapsed_ms = (((end - start)*100000)//1)/100
print(f"The sum of first three values of the solved sudokus is: {grid_sum}.\nThis took {elapsed_ms}ms.")